...
@onready var rect: TextureRect = find_child("TextureRect")
...
func Get_Breite(Pixel):
	var Breite =  3 #int(sqrt(CONWAYSVARIABEL.Pixel))
	return Breite
...
func _ready():
	if rect == null:
		print("rect ist null – TextureRect nicht gefunden!", true)
		return
	print("rect ist gültig", true)
